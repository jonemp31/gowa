...
type IChatStorageRepository interface {
	...
	GetMessageByID(id string) (*Message, error)                        // Cross-device lookup (intentional for revoke/reaction)
	GetMessageByIDForDevice(id, deviceID string) (*Message, error)    // Device-scoped lookup (reply context, delete-for-me)
