...
func (m *DeviceManager) EnsureClient(ctx context.Context, deviceID string) (*DeviceInstance, error) {
	...
	// Slow path: acquire init lock so concurrent callers queue up.
	// Unlike sync.Once this allows a retry when the previous attempt failed.
	inst.clientMu.Lock()
	...
	// Reset previous error so each locked attempt is a fresh try.
